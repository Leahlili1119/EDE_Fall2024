...
output: 
  pdf_document:
    ...
    number_sections: false # Disables section numbering
    fig_caption: true     # Ensures figure captions are displayed
    keep_tex: true        # Keeps the intermediate LaTeX file
lof: true                 # Adds List of Figures
---
...
# Display the PM2.5 table
kable(summary_PM25, col.names = c("County", "$\\mu g/m^3$"), 
      caption = "Mean Particulates (2.5mm)")

# Display the Ozone table
kable(summary_Ozone, col.names = c("County", "$\\mu g/m^3$"), 
      caption = "Mean Ozone")
```
...
```{r ozone_2018, echo=FALSE, warning=FALSE, fig.align="left", fig.height=3, fig.cap="2018 monthly Ozone levels"}
library(ggplot2)

# Filter the data for 2018
ozone_2018 <- epa_data %>% filter(year(Date) == 2018)

# Create the boxplot for 2018
ggplot(ozone_2018, aes(x = factor(month(Date)), y = Ozone)) +
  geom_boxplot() +
  labs(x = "Month", y = "Ozone Levels (ppb)", title = "Ozone Levels by Month (2018)")
```

```{r ozone_2019, echo=FALSE, warning=FALSE, fig.align="left", fig.height=3, fig.cap="2019 monthly Ozone levels"}
# Filter the data for 2019
ozone_2019 <- epa_data %>% filter(year(Date) == 2019)

# Create the boxplot for 2019
ggplot(ozone_2019, aes(x = factor(month(Date)), y = Ozone)) +
  geom_boxplot() +
  labs(x = "Month", y = "Ozone Levels (ppb)", title = "Ozone Levels by Month (2019)")
```
...
## Git Repository
https://github.com/Leahlili1119/EDE_Fall2024
